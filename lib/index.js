...
// make path able to go backward
var _preparePath = function _preparePath(path){
  if (typeof path === "string") {
    path = path.split(".");
  }

  var backwardIndex = path.findIndex(function(el){ return el == '<<' });
  while(typeof backwardIndex != "undefined" && backwardIndex != -1){
    path.splice(backwardIndex-1, 2);
    backwardIndex = path.findIndex(function(el){ return el == '<<' });
  }
...
var _exists = function _exists(object, path){
...
var _put = function _put(pbject, path, value){
  if (!(path instanceof Array) || path.length === 0) {
    return false;
  }
