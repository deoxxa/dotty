...
// an object, it will not be overwritten, unless the `force` argument is set to 
// `true`. Explicitly set `force` to `false` if you do *not* want to overwrite
// an existing value.
//
...
var put = module.exports.put = function put(object, path, value, force) {
  if (typeof path === "string") {
    path = path.split(".");
  }
  ...
  if (path.length === 0) {
    if (object[key] !== undefined && force === false)
      return false;
    else
      object[key] = value;
  } else {
    ...
    return put(object[key], path, value, force);
  }
};

//
// Synonym for people who are into getters and setters
//

module.exports.set = put; // synonyms
